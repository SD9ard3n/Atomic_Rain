...
description: 核心原则: 当 AI 遇到自动化无法覆盖 / 需要凭证 / 需要人眼判断的节点时, 必须停下来, 明确告诉用户"需要做什么、需要提供什么", 用户复现后回传给 AI 继续。 避免 AI 瞎猜 / 编造结果 / 跳过关键步骤。
...
**阻塞型 HITL 规则**:
- 如果缺少账号、登录态、Cookie、Authorization、验证码、两测试账号、OOB 域名、接收邮箱/手机号、webhook、外链、VPS/listener 或云 AK 授权,先发 HITL 请求并等待。
- 不得把"需要用户资源"解释成"先跳过这个点测别的"。只有用户明确说跳过、稍后提供、没有资源、或同意降级/公共资源时,才能改测其他路径。
- 用户未回复时,记录 `[WAITING:HITL_REQUIRED]`;用户拒绝或无资源时,记录 `[BLOCKED:需要用户资源]`。
