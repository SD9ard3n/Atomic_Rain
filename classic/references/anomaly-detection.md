...
```python
# 使用 curl -i 获取响应头并解析
response_raw = subprocess.check_output(['curl', '-i', '-s', url]).decode()
response_headers = parse_headers(response_raw)  # 自行解析响应头
...
durations = []
for i in range(20):
    # 使用 curl 或 httpx 发送请求
    result = subprocess.check_output(['curl', '-i', '-w', '
...
**版本历史**:
- v2.0 (2026-06-08): toolPlus 初始版本, 集成 curl/httpx CLI
