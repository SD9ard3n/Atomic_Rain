...
- Discovery-Origin: **(必填)** 这个接口是怎么发现的
  例: "Phase 1 JS分析 /app.js 暴露路径 /api/v1/admin"
  例: "Phase 2 对 /api/v1/user 参数 fuzz 时发现"
- Repro-Request: **(必填)** 完整 BP 格式请求包 (替代单行 curl)
  ```
  ...
  {"param":"payload"}
  ```
- Chain-Steps: **(必填)** 利用链每一步的完整请求包+响应摘要
  Step 1: <请求包> → <响应摘要>
  Step 2: <请求包> → <响应摘要>
  实际影响: <读到/改到/接管了什么>
- Evidence Chain:
...
**Phase 4 报告收尾时**:
- 仍是 `[待验证-High/Critical]` 且补不上 Chain-Steps → 降为 Medium (收尾清理,不是过程中卡断)
...
不要为了凑字段编 Chain-Steps,空着就空着、加 `[待验证]` 标签即可。
...
- **强制动作**:
  1. 满足 → 标 `[Confirmed]`
  2. 在 `vulns.md` 中填写 `Discovery-Origin` (接口是怎么发现的: Phase/步骤/来源)
  3. 在 `vulns.md` 中填写 `Repro-Request` (产生信号时的完整 HTTP 请求/响应对,含全部 Header/Cookie/Body,不是事后补的单行 curl)
  4. 如触发级联: 每一步级联操作的完整请求包填入 `Chain-Steps`
  > **原则**: 信息在发现时就地捕获, 不依赖 Phase 4 事后回忆
