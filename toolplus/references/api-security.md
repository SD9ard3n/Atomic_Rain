...
- 先完成 API 来源归因: JS、移动流量、miniapp、OpenAPI/Swagger、GraphQL、历史流量或后台接口。
- BOLA/BFLA 优先走 [business-flow-checklist.md](business-flow-checklist.md)、[resource-classification.md](resource-classification.md) 和两账号对照, 不从 payload 列表开始。
- GraphQL、JWT、OAuth、CORS 等专项信号出现后, 跳转到对应 vuln 文件做深测, 本文只保留 API 路由和证据门槛。
- 批量枚举、速率限制、弱口令和敏感操作测试必须有范围、频率、账号边界和停止条件。
- 报告前保留 endpoint、method、auth context、对象 owner、状态/长度/时间差异和 negative control。
...
付费视频、课程、专栏和 CDN 对象资源必须证明未购买/未登录可访问、鉴权缺失、过期失效或可预测批量访问; 试看地址和公开预览不算漏洞。
完整失败转向、评级边界和误判过滤见 [src-failure-pivots.md §API 资源与状态变更](src-failure-pivots.md)。

### 11.2 取消/删除/撤回接口

取消、删除、撤回类接口只用 A/B 自建对象验证归属、sign 行为和最终状态; `success` 或 sign 报错都不能替代服务端状态检查。
完整失败转向、评级边界和误判过滤见 [src-failure-pivots.md §API 资源与状态变更](src-failure-pivots.md)。
