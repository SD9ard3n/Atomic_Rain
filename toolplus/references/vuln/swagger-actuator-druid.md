...
| `/druid/index.html` / `/druid/login.html` 200 | Druid 监控暴露 | → [druid-console.md](druid-console.md) |
...
| Druid 需登录 | 默认 admin/admin | → [druid-console.md](druid-console.md) |
...
- 完整横向评级、HITL 和最小证据包见 [exposed-console-evidence-boundaries.md](exposed-console-evidence-boundaries.md)。

### False Positive Gate

- 排除仅返回 health/up、404 fallback、登录页或样式资源的情况。
- masked env (`***`) 不能当作明文泄露。
- 默认凭据必须证明登录成功或接口可用, 不能只列账号密码字典。完整误判过滤见 [exposed-console-evidence-boundaries.md](exposed-console-evidence-boundaries.md)。
...
Druid 默认凭据、未授权、SQL 监控、数据源连接串和 WebSession 接管已迁移到 [druid-console.md](druid-console.md)。本文件保留 Swagger / Actuator / 暴露控制台总入口。
...
### 6.3 Druid
- [ ] 进入 [druid-console.md](druid-console.md),按 Druid 专项最小证据流程验证。
...
暴露控制台误判过滤已抽到 [exposed-console-evidence-boundaries.md](exposed-console-evidence-boundaries.md)。本文件只保留 endpoint 和利用路径;报告前必须区分静态 UI、health/up、masked env、空面板、登录页和真实敏感数据/可调用能力。

---

## 8. 影响证明

完整评级口径见 [exposed-console-evidence-boundaries.md](exposed-console-evidence-boundaries.md)。快速口径: 静态 UI/health/up/登录页通常低;OpenAPI JSON、mappings、beans 或非敏感配置为低中;明文凭证、SQL 参数、Session、连接串为 High 候选;Jolokia/H2/Gateway/Actuator 写入、命令执行、云/K8s 凭据为 Critical 候选。
...
- Druid 监控专项 → [druid-console.md](druid-console.md)
