---
name: csrf-clickjacking
description: CWE: CWE-352 (CSRF) / CWE-1021 (Clickjacking) OWASP: WSTG-SESS-05 / WSTG-CLNT-09 / A01:2021
category: vuln
tags: [client]
---
...
- 改密、改邮箱、绑定 OAuth、转账、下单、删除、授权、上传等状态改变接口。
- Cookie SameSite、CSRF Token、Origin/Referer 校验、JSON/simple request 差异。
- 可被 iframe 加载的关键页面、弹窗确认和组合型 UI redressing。

### Pro Tips

- 先证明操作真实生效, 不要只看 HTTP 200。
- 用低权限测试账号生成最小 PoC, 避免真实用户和高价值资产。
- Clickjacking 要同时检查 `X-Frame-Options` 和 CSP `frame-ancestors`。

### Evidence / Rating Boundary

- 无敏感状态变化的 CSRF/Clickjacking 通常低危或不收。
- 改密、绑定第三方、支付/转账、授权变更等可按账号/资金影响定级。
- JSON CSRF、CORS 反射、XSS 组合链按实际可达影响升级。

### False Positive Gate

- 删 token 返回 200 但状态不变是误报。
- SameSite/Origin/Referer 任一有效防护可能阻断真实跨站利用。
- iframe 可加载但没有可诱导的敏感动作, 不应高估。
