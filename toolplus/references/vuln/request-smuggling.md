...
description: 定位: 前后端 HTTP 解析不一致, 攻击者插入 "第二请求" 被后端误认为独立请求。 CWE: CWE-444 | OWASP: A05:2021 Security Misconfiguration 回报: Cloudflare/AWS/CDN 背后的业务, $2000-$…
category: vuln
tags: [server]
---
...
- CDN/反向代理/WAF/负载均衡后面的 HTTP/1.1 服务。
- `Content-Length`、`Transfer-Encoding`、HTTP/2 downgrade、连接复用和缓存层。
- 管理路径、登录态路径、缓存路径和可反射路径。

### Pro Tips

- First-pass 只做 timing/差异探测, 不污染缓存、不投递跨用户 payload。
- 在受控路径、受控账号和短连接窗口内复测, 记录前后端响应差异。
- 发现疑似走私后先停手确认范围, 再做最小化影响证明。

### Evidence / Rating Boundary

- 单次超时或 408 只是弱信号。
- 稳定 timing 差异 + 可控前缀/后缀响应可作为中高置信证据。
- 缓存投毒、鉴权绕过、请求劫持或跨用户影响需要受控环境/HITL 后再定高危。

### False Positive Gate

- 排除网络抖动、代理限速、WAF challenge、正常 keep-alive 超时。
- 必须多轮基线对照, 不能用一次延迟定性。
- 不报告会影响第三方用户但无法复现的污染性结果。
