---
name: deserialize
description: CWE: CWE-502 | OWASP: WSTG-INPV-11 / A08:2021 核心: 用户可控的序列化数据被反序列化时触发 gadget 链 → RCE。 回报: 严重级 RCE, $3000-$30000+
category: vuln
tags: [deser]
---
...
- Cookie、Header、请求体、上传文件、消息队列、缓存字段中的序列化对象。
- Java / PHP / Python / .NET / Ruby / Node.js 格式魔数、框架包装格式和错误堆栈。
- Shiro、Fastjson/Jackson、XStream/Hessian/Dubbo 等反序列化入口。

### Pro Tips

- 先做格式指纹和非破坏探测, 再按运行时选择 gadget 家族。
- OOB/URLDNS 必须使用已批准通道; 未批准前只记录 blocked。
- 不在 First-pass 阶段做写文件、反弹 shell、持久化或破坏性 gadget。

### Evidence / Rating Boundary

- 仅有魔数、500 或可解码对象是线索, 不能直接定高危。
- OOB 命中、稳定延迟或受控命令回显可证明反序列化执行路径。
- RCE、文件写入、云凭证利用等影响证明必须走 HITL 最小化验证。
- 完整横向评级和最小证据包见 [deserialization-evidence-boundaries.md](deserialization-evidence-boundaries.md)。

### False Positive Gate

- 排除普通 Base64、压缩包、加密 token 或仅客户端编码。
- 排除扫描器/代理自身触发的 OOB 噪声。
- 报告前必须说明入口、格式、触发点、gadget 选择和可重复证据;完整误判过滤见 [deserialization-evidence-boundaries.md](deserialization-evidence-boundaries.md)。
...
反序列化族误判过滤已抽到 [deserialization-evidence-boundaries.md](deserialization-evidence-boundaries.md)。本文件只保留核心提醒: 魔数、500、可解码对象、ClassNotFound、单次慢响应或空 OOB 记录都不能直接定级;必须证明入口、格式、触发点、可控 payload 和可重复影响。

---

## 9. 影响证明

完整评级边界见 [deserialization-evidence-boundaries.md](deserialization-evidence-boundaries.md)。快速口径:

- 线索: 魔数、可解码对象、500、ClassNotFound。
- 强信号: 授权 OOB 命中、稳定延迟、可重复消费链。
- confirmed: 受控最小命令回显、明确服务端执行路径或可证明状态变化。
- Critical 候选: 文件写入、WebShell、云凭据、批量数据或多实例影响;必须先走 HITL 最小化验证。
