...
description: OWASP: LLM02 (Sensitive Information Disclosure) / LLM04 (Data Poisoning) / LLM07 (System Prompt Leakage) / LLM08 (Vector Weaknesses) CWE: C…
...
- 本文只在出现数据泄露、RAG、向量库、成员推断、训练数据或隐私信号时加载; 普通 Prompt 注入先读 [ai-app-security.md](ai-app-security.md)。
- 先判定数据面: system prompt、用户会话、上传文件、RAG 知识库、训练样本、向量库或插件缓存, 再读对应章节。
- 不把模型"愿意回答"等同于真实泄露; 需要可复现样本、上下文来源、跨用户边界或知识库污染路径。
- 成员推断、模型反演和训练数据推导容易误判, 默认作为 candidate, 需要统计对照或目标方确认才能升级。
- 涉及真实用户数据时最小化输出并脱敏, 报告中区分已证实数据、推断数据和潜在风险。
