...
短信/验证码横向评级、HITL 和误判过滤见 [auth-evidence-boundaries.md](auth-evidence-boundaries.md)。本文件只保留认证链路和具体测试入口; 定级前必须证明最终注册、登录、改密、绑定或业务状态变化。
...
弱口令从“默认清单”升级为 `规则来源 -> 授权验证 -> 权限范围 -> 同款迁移`。完整证据边界见 [auth-evidence-boundaries.md](auth-evidence-boundaries.md); 本文件只保留入口和组合链。不得输出真实账号密码。
...
默认口令不在本文件维护字典。按 [weak-password-generation.md](weak-password-generation.md) 生成目标上下文规则,并用 [auth-evidence-boundaries.md](auth-evidence-boundaries.md) 约束授权范围、频率、证据和报告脱敏。
...
支付、优惠券、限购、订阅、试用、退款、核销、积分商城、响应价格信任和异步校验统一走 [src-business-logic-state-machine.md](src-business-logic-state-machine.md)。本文件只保留认证侧入口: 谁能发起、谁能支付、谁能退款、谁能核销、最终权益属于谁。

关键证据: 正常基线、修改请求、负向控制、最终服务端订单/权益/余额/券/积分状态。真实金额、退款、提现和高价值权益必须 HITL。

---

## 7. 条件竞争

条件竞争专项已由 [vuln/race-condition.md](vuln/race-condition.md) 承接。认证场景只保留入口: 密码重置 token、邮箱修改、验证码、Session、优惠券、积分、支付、转账和库存扣减。并发测试必须控制规模,真实金额/转账/退款先 HITL。
...
| 支付/业务状态 | 提现/退款 → 资金损失 | [src-business-logic-state-machine.md](src-business-logic-state-machine.md) |
...
| 条件竞争(领券) | 多张优惠券 → 支付时叠加使用 | [vuln/race-condition.md](vuln/race-condition.md) + [src-business-logic-state-machine.md](src-business-logic-state-machine.md) |
