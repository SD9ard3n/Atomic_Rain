...
def check_toolplus_mcp_readiness(errors: list[str], profile: str) -> None:
    if profile not in {"toolplus", "deployed-mixed"}:
        return
    readiness = ROOT / "references" / "mcp-readiness.md"
    if not readiness.exists():
        errors.append("toolPlus missing MCP readiness checklist: references/mcp-readiness.md")
        return
    readiness_text = read_text(readiness)
    required_terms = {
        "http://127.0.0.1:11432/sse": "Yakit SSE endpoint",
        "http://127.0.0.1:12306/mcp": "Chrome streamable HTTP endpoint",
        "get_current_database_context": "Yakit database readiness check",
        "get_windows_and_tabs": "Chrome tab readiness check",
        "server name": "actual MCP server-name discovery",
        "tool namespace": "actual tool namespace discovery",
        "DEGRADED:YAKIT_MCP_DOWN": "Yakit degraded marker",
        "DEGRADED:CHROME_MCP_DOWN": "Chrome degraded marker",
        "DEGRADED:SSA_UNAVAILABLE": "SSA degraded marker",
    }
    for term, label in required_terms.items():
        if term not in readiness_text:
            errors.append(f"mcp-readiness.md missing {label}: {term}")

    for relpath in ("SKILL.md", "references/protocols/agent-protocol.md", "references/tool-config.md"):
        text = read_text(ROOT / relpath)
        if "mcp-readiness.md" not in text:
            errors.append(f"{relpath} should route MCP readiness decisions to references/mcp-readiness.md")
    ...
    if "无法降级" in combined or "禁止降级跑" in combined:
        errors.append("toolPlus MCP failure policy still contains one-size-fits-all no-degrade wording")
...
def main() -> int:
    ...
    check_toolplus_mcp_readiness(errors, profile)
